...
// Data - The Chimera Company branded content
const topContent = ['Bespoke Events', 'Luxury Management', 'Bespoke Events', 'Luxury Management']
const bottomContent = ['Detail & Discretion', 'Destination Weddings', 'Corporate Excellence', 'Detail & Discretion', 'Destination Weddings']

const ClientsV3 = () => {
  return (
    <section className="overflow-hidden pb-14 pt-14 md:pb-16 md:pt-16 ">
      <RevealWrapper>
        <Marquee pauseOnHover>
          <div className="flex items-center justify-between py-2.5 pb-5">
            {topContent?.map((content, idx) => (
              <div className="mr-5 flex items-center gap-6" key={`top-${idx}`}>
        ...
        <Marquee direction="right" pauseOnHover>
          <div className="flex items-center justify-between py-2.5">
            {bottomContent?.map((content, idx) => (
              <div className="mr-5 flex items-center gap-6" key={`bottom-${idx}`}>
                <span className="text-nowrap text-5xl font-medium leading-tight tracking-[-2px] sm:text-[55px] md:text-[67px] lg:text-[84px] xl:text-8xl xl:leading-[1.1] xl:tracking-[-2.88px]">
                  {content}
                </span>
                <span className="mt-5">
